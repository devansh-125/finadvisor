...
class FinancialAnalytics {
  ...
  async generateAnalytics(analysis, rules, user) {
    ...
    try {
      ...
      // Use insights from rules (already generated by RuleEngine)
      ...
      const savingsRateObj = kpis.savingsRate || { rate: 0, status: 'poor' };
      const savingsRate = typeof savingsRateObj.rate === 'number' ? savingsRateObj.rate : 0;
      let savingsRateStatus = savingsRateObj.status || 'poor';
      ...
      const expenseRatioObj = kpis.expenseRatio || { ratio: 100, status: 'poor' };
      const expenseRatio = typeof expenseRatioObj.ratio === 'number' ? expenseRatioObj.ratio : 100;
      let expenseRatioStatus = expenseRatioObj.status || 'poor';
      ...
      return {
        ...
        summary: {
          ...
          riskLevel: (kpis.financialRiskScore?.score || 50) < 30 ? 'Low' : (kpis.financialRiskScore?.score || 50) < 70 ? 'Medium' : 'High',
